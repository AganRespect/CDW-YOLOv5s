...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        #self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()  ###原始激活函数
        #self.act = FReLU(c2) if act is True else (act if isinstance(act, nn.Module) else nn.Identity())        ###FReLU
